...
import { 
  BarChart3, TrendingUp, Clock, Users, Download, AlertCircle, CheckCircle2, 
  Timer, Award, Zap, Calendar, Activity, Target 
} from 'lucide-react';
...
const ProjectReports = ({ projectId }: ProjectReportsProps) => {
  ...
  const [reportData, setReportData] = useState<ReportData>({
    projectStats: { avgCycleTime: 0, throughput: 0 },
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const { data: allIssuesData } = await supabase
        .from('issues')
        .select('*, assignee:profiles!issues_assignee_id_fkey(user_id, full_name, email)')
      ...
      const { data: timeLogsData } = await supabase
        .from('time_logs')
        .select('*, user:profiles(user_id, full_name, email), issue:issues!inner(project_id)')
      ...
      // Fetch all profiles involved in this project
  ...
  const generateAdvancedTeamPerformance = (issues: any[], timeLogs: any[], profiles: any[]): UserPerformance[] => {
    ...
    // Initialize team members who have been assigned issues or logged time
    const involvedUserIds = new Set([
      ...issues.filter(i => i.assignee_id).map(i => i.assignee_id),
      ...timeLogs.map(t => t.user_id)
    ]);
    ...
    return Array.from(teamMap.values()).sort((a, b) => b.efficiency - a.efficiency);
  };

  const generateWorkloadDistribution = (issues: any[], profiles: any[]) => {
    const workloadMap = new Map();
    
    // Only include profiles that are involved in this project
    const involvedUserIds = new Set(issues.filter(i => i.assignee_id).map(i => i.assignee_id));
    ...
    issues.filter(issue => issue.status !== 'done').forEach(issue => {
      if (issue.assignee_id && workloadMap.has(issue.assignee_id)) {
        const estimatedHours = issue.estimated_hours || 8;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Comprehensive insights and metrics for this project
          </p>
        </div>
        <div className="flex items-center space-x-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuItem onClick={() => exportToWord(reportData, 'Project')}>
                Export as Word (.docx)
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => exportToExcel(reportData, 'Project')}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-2">{reportData.issueStats.total}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-2">
              {reportData.issueStats.byStatus['done'] || 0}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-2">
              {reportData.issueStats.byStatus['in_progress'] || 0}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-2">{reportData.issueStats.overdueTasks}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-2">{reportData.projectStats.avgCycleTime}d</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-2">{reportData.projectStats.throughput}</p>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={reportData.burndownData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="remaining" stroke="#8884d8" name="Remaining" />
                  <Line type="monotone" dataKey="ideal" stroke="#82ca9d" strokeDasharray="5 5" name="Ideal" />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
